...
async def test_on_cancel_task_success():
    mock_task_store = AsyncMock(spec=TaskStore)
    sample_task = create_sample_task(task_id='task_to_cancel')
    mock_task_store.get.return_value = sample_task
    request_handler = DefaultRequestHandler(
        agent_executor=AsyncMock(spec=AgentExecutor), task_store=mock_task_store
    )
    mock_active_task = AsyncMock()
    mock_active_task.wait.return_value = create_sample_task(
        task_id='task_to_cancel', status_state=TaskState.TASK_STATE_CANCELED
    )
    request_handler._active_task_registry.get_or_create = AsyncMock(
        return_value=mock_active_task
    )
    context = create_server_call_context()
    params = CancelTaskRequest(id='task_to_cancel')
    result_task = await request_handler.on_cancel_task(params, context)
    mock_task_store.get.assert_awaited_once_with('task_to_cancel', context)
    request_handler._active_task_registry.get_or_create.assert_awaited_once_with(
        'task_to_cancel'
    )
    mock_active_task.cancel.assert_awaited_once()
    assert result_task is not None
    assert result_task.status.state == TaskState.TASK_STATE_CANCELED


@pytest.mark.asyncio
async def test_on_cancel_task_task_in_terminal_state():
    """Test on_cancel_task when task is already in a terminal state."""
    mock_task_store = AsyncMock(spec=TaskStore)
    sample_task = create_sample_task(
        task_id='task_already_canceled',
        status_state=TaskState.TASK_STATE_CANCELED,
    )
    mock_task_store.get.return_value = sample_task
    request_handler = DefaultRequestHandler(
        agent_executor=AsyncMock(spec=AgentExecutor), task_store=mock_task_store
    ...
    context = create_server_call_context()
    params = CancelTaskRequest(id='task_already_canceled')
    result_task = await request_handler.on_cancel_task(params, context)
...
async def test_on_cancel_task_completes_during_cancellation():
    ...
    mock_active_task = AsyncMock()
    mock_active_task.wait.return_value = create_sample_task(
        task_id='task_to_cancel', status_state=TaskState.TASK_STATE_COMPLETED
    )
    request_handler._active_task_registry.get_or_create = AsyncMock(
        return_value=mock_active_task
    )
    ...
    result_task = await request_handler.on_cancel_task(params, context)
    assert result_task is not None
    assert result_task.status.state == TaskState.TASK_STATE_COMPLETED
...
async def test_on_cancel_task_invalid_result_type():
    ...
    mock_active_task = AsyncMock()
    mock_active_task.wait.return_value = Message(role=Role.ROLE_AGENT)
    request_handler._active_task_registry.get_or_create = AsyncMock(
        return_value=mock_active_task
    )
    context = create_server_call_context()
...
async def test_stream_disconnect_then_resubscribe_receives_future_events():
    ...
    resub_gen = request_handler.on_subscribe_to_task(
        SubscribeToTaskRequest(id=f'{task_id}'), create_server_call_context()
    )
...
async def test_on_message_send_task_id_provided_but_task_not_found():
    ...
    with (
        ...
        pytest.raises(TaskNotFoundError) as exc_info,
    ):
        await request_handler.on_message_send(
            params, create_server_call_context()
        )
...
async def test_on_message_send_stream_task_id_mismatch():
    ...
    params = SendMessageRequest(
        message=Message(
            role=Role.ROLE_USER, message_id='msg_id_mismatch_stream', parts=[]
        )
    )
