...
class MockAgentExecutor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue):
            ...
            async def emit_periodic():
                try:
                    while not event.is_set():
                        await task_updater.update_status(TaskState.working, message=task_updater.new_agent_message([Part(root=TextPart(text='ping'))]))
        ...
        await task_updater.update_status(
            TaskState.completed,
            message=task_updater.new_agent_message([Part(root=TextPart(text='done'))], metadata={"response_key": "response_value"}),
        )
...
async def main_async(http_port: int, grpc_port: int):
    ...
    app = FastAPI()
    app.mount("/jsonrpc", A2AFastAPIApplication(http_handler=handler, agent_card=agent_card).build())
    app.mount("/rest", A2ARESTFastAPIApplication(http_handler=handler, agent_card=agent_card).build())
