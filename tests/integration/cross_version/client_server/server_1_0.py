...
from a2a.compat.v0_3.server_adapter import Compat03RESTFastAPIApplication
...
class MockAgentExecutor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue):
        ...
        metadata = dict(context.message.metadata) if context.message and context.message.metadata else {}
        if metadata.get("test_key") != "test_value":
            print(f"SERVER: WARNING: Missing or incorrect metadata: {metadata}")
            raise ValueError(f"Missing expected metadata from client. Got: {metadata}")
            ...
            async def emit_periodic():
                try:
                    while not event.is_set():
                        await task_updater.update_status(TaskState.TASK_STATE_WORKING, message=task_updater.new_agent_message([Part(text='ping')]))
        ...
        await task_updater.update_status(
            ...
            message=task_updater.new_agent_message([Part(text='done')], metadata={"response_key": "response_value"}),
        )
...
async def main_async(http_port: int, grpc_port: int):
    ...
    jsonrpc_app = A2AFastAPIApplication(http_handler=handler, agent_card=agent_card).build()
    app.mount("/jsonrpc", jsonrpc_app)
    app.mount("/rest", Compat03RESTFastAPIApplication(http_handler=handler, agent_card=agent_card).build())
    ...
    from a2a.compat.v0_3.grpc_adapter import mount_compat_grpc_servicer
    mount_compat_grpc_servicer(server, handler)
