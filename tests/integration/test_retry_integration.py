"""Integration tests for RetryTransport through the full client stack.

Tests RetryTransport composed with ClientFactory/BaseClient against
real Starlette servers, validating the full client -> retry -> transport -> server
path that end users would follow.
"""
...
def _wrap_with_transient_503(app, fail_count: int = 2):
    """Wraps an ASGI app to return 503 for the first N requests.

    Returns a tuple of (middleware_app, state_dict) where state_dict['count']
    tracks how many requests were intercepted.
    """
    state = {'count': 0}
...
def mock_request_handler() -> AsyncMock:
    """Provides a mock RequestHandler with retry-relevant responses."""
    handler = AsyncMock(spec=RequestHandler)

    # Configure responses for retry tests
...
def agent_card() -> AgentCard:
    """Provides a sample AgentCard for retry integration tests."""
    return AgentCard(
...
async def test_retry_with_client_factory_rest(
    ...
    """Full stack: ClientFactory -> BaseClient -> RetryTransport -> RestTransport -> server."""
    ...
    # Wrap the transport with RetryTransport
...
async def test_retry_with_client_factory_jsonrpc(
    ...
    """Full stack: ClientFactory -> BaseClient -> RetryTransport -> JsonRpcTransport -> server."""
...
async def test_retry_send_message_blocking(
    ...
    """Full stack: send_message through RetryTransport with transient failures."""
    rest_routes = create_rest_routes(
        agent_card, mock_request_handler, extended_agent_card=agent_card
    )
    app = Starlette(routes=[*rest_routes])
