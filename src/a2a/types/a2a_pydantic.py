...
class AuthenticationInfo(A2ABaseModel):
    ...
    scheme: str = Field(..., description='HTTP Authentication Scheme from the [IANA registry](https://www.iana.org/assignments/http-authschemes/). Examples: `Bearer`, `Basic`, `Digest`. Scheme names are case-insensitive per [RFC 9110 Section 11.1](https://www.rfc-editor.org/rfc/rfc9110#section-11.1).')
    credentials: str = Field(default='', description='Push Notification credentials. Format depends on the scheme (e.g., token for Bearer).')
...
class TaskPushNotificationConfig(A2ABaseModel):
    ...
    id: str = Field(default='', description='The push notification configuration details. A unique identifier (e.g. UUID) for this push notification configuration.')
...
class Message(A2ABaseModel):
    ...
    message_id: str = Field(..., description='The unique identifier (e.g. UUID) of the message. This is created by the message creator.')
    context_id: str = Field(default='', description='Optional. The context id of the message. If set, the message will be associated with the given context.')
...
class Task(A2ABaseModel):
    ...
    id: str = Field(..., description='Unique identifier (e.g. UUID) for the task, generated by the server for a new task.')
    context_id: str = Field(default='', description='Unique identifier (e.g. UUID) for the contextual collection of interactions (tasks and messages).')
...
class Artifact(A2ABaseModel):
    ...
    artifact_id: str = Field(..., description='Unique identifier (e.g. UUID) for the artifact. It must be unique within a task.')
...
class TaskArtifactUpdateEvent(A2ABaseModel):
    ...
    task_id: str = Field(..., description='The ID of the task for this artifact.')
    context_id: str = Field(..., description='The ID of the context that this task belongs to.')
...
class AgentInterface(A2ABaseModel):
    ...
    url: str = Field(..., description='The URL where this interface is available. Must be a valid absolute HTTPS URL in production. Example: "https://api.example.com/a2a/v1", "https://grpc.example.com/a2a"')
    protocol_binding: str = Field(..., description='The protocol binding supported at this URL. This is an open form string, to be easily extended for other protocol bindings. The core ones officially supported are `JSONRPC`, `GRPC` and `HTTP+JSON`.')
    tenant: str = Field(default='', description='Tenant ID to be used in the request when calling the agent.')
    protocol_version: str = Field(..., description='The version of the A2A protocol this interface exposes. Use the latest supported minor version per major version. Examples: "0.3", "1.0"')
...
class AgentCapabilities(A2ABaseModel):
    ...
    streaming: bool | None = Field(default=None, description='Indicates if the agent supports streaming responses.')
    push_notifications: bool | None = Field(default=None, description='Indicates if the agent supports sending push notifications for asynchronous task updates.')
    extensions: list[AgentExtension] | None = Field(default=None, description='A list of protocol extensions supported by the agent.')
    extended_agent_card: bool | None = Field(default=None, description='Indicates if the agent supports providing an extended agent card when authenticated.')
...
class AgentCard(A2ABaseModel):
    ...
    security_schemes: dict[str, SecurityScheme] | None = Field(default=None, description='The security scheme details used for authenticating with this agent.')
    security_requirements: list[SecurityRequirement] | None = Field(default=None, description='Security requirements for contacting the agent.')
    default_input_modes: list[str] = Field(..., description='protolint:enable REPEATED_FIELD_NAMES_PLURALIZED The set of interaction modes that the agent supports across all skills. This can be overridden per skill. Defined as media types.')
    default_output_modes: list[str] = Field(..., description='The media types supported as outputs from this agent.')
    skills: list[AgentSkill] = Field(..., description='Skills represent the abilities of an agent. It is largely a descriptive concept but represents a more focused set of behaviors that the agent is likely to succeed at.')
    signatures: list[AgentCardSignature] | None = Field(default=None, description='JSON Web Signatures computed for this `AgentCard`.')
    icon_url: str | None = Field(default=None, description='Optional. A URL to an icon for the agent.')
...
class StringList(A2ABaseModel):
    ...
    list_: list[str] | None = Field(default=None, alias='list', description='The individual string values.')
...
class SecurityRequirement(A2ABaseModel):
    ...
    schemes: dict[str, StringList] | None = Field(default=None, description='A map of security schemes to the required scopes.')
...
class APIKeySecurityScheme(A2ABaseModel):
    ...
    location: str = Field(..., description='The location of the API key. Valid values are "query", "header", or "cookie".')
...
class HTTPAuthSecurityScheme(A2ABaseModel):
    ...
    description: str = Field(default='', description='An optional description for the security scheme.')
    scheme: str = Field(..., description='The name of the HTTP Authentication scheme to be used in the Authorization header, as defined in RFC7235 (e.g., "Bearer"). This value should be registered in the IANA Authentication Scheme registry.')
    bearer_format: str = Field(default='', description='A hint to the client to identify how the bearer token is formatted (e.g., "JWT"). Primarily for documentation purposes.')
...
class OAuthFlows(A2ABaseModel):
    ...
    flow: AuthorizationCodeOAuthFlow | ClientCredentialsOAuthFlow | ImplicitOAuthFlow | PasswordOAuthFlow | DeviceCodeOAuthFlow | None = None
...
class OAuth2SecurityScheme(A2ABaseModel):
    ...
    description: str = Field(default='', description='An optional description for the security scheme.')
    flows: OAuthFlows = Field(..., description='An object containing configuration information for the supported OAuth 2.0 flows.')
    oauth2_metadata_url: str = Field(default='', description='URL to the OAuth2 authorization server metadata [RFC 8414](https://datatracker.ietf.org/doc/html/rfc8414). TLS is required.')
...
class AuthorizationCodeOAuthFlow(A2ABaseModel):
    ...
    authorization_url: str = Field(..., description='The authorization URL to be used for this flow.')
    token_url: str = Field(..., description='The token URL to be used for this flow.')
    refresh_url: str = Field(default='', description='The URL to be used for obtaining refresh tokens.')
    scopes: dict[str, str] = Field(..., description='The available scopes for the OAuth2 security scheme.')
    pkce_required: bool = Field(default=False, description='Indicates if PKCE (RFC 7636) is required for this flow. PKCE should always be used for public clients and is recommended for all clients.')
...
class ImplicitOAuthFlow(A2ABaseModel):
    ...
    authorization_url: str = Field(default='', description='The authorization URL to be used for this flow. This MUST be in the form of a URL. The OAuth2 standard requires the use of TLS')
    refresh_url: str = Field(default='', description='The URL to be used for obtaining refresh tokens. This MUST be in the form of a URL. The OAuth2 standard requires the use of TLS.')
    scopes: dict[str, str] | None = Field(default=None, description='The available scopes for the OAuth2 security scheme. A map between the scope name and a short description for it. The map MAY be empty.')
...
class PasswordOAuthFlow(A2ABaseModel):
    ...
    token_url: str = Field(default='', description='The token URL to be used for this flow. This MUST be in the form of a URL. The OAuth2 standard requires the use of TLS.')
    refresh_url: str = Field(default='', description='The URL to be used for obtaining refresh tokens. This MUST be in the form of a URL. The OAuth2 standard requires the use of TLS.')
    scopes: dict[str, str] | None = Field(default=None, description='The available scopes for the OAuth2 security scheme. A map between the scope name and a short description for it. The map MAY be empty.')
...
class DeviceCodeOAuthFlow(A2ABaseModel):
    ...
    device_authorization_url: str = Field(..., description='The device authorization endpoint URL.')
    token_url: str = Field(..., description='The token URL to be used for this flow.')
    refresh_url: str = Field(default='', description='The URL to be used for obtaining refresh tokens.')
    scopes: dict[str, str] = Field(..., description='The available scopes for the OAuth2 security scheme.')
...
class ListTasksResponse(A2ABaseModel):
    ...
    next_page_token: str = Field(..., description='A token to retrieve the next page of results, or empty if there are no more results in the list.')
    page_size: int = Field(..., description='The page size used for this response.')
    total_size: int = Field(..., description='Total number of tasks available (before pagination).')
...
class GetTaskPushNotificationConfigRequest(A2ABaseModel):
    ...
    id: str = Field(..., description='The resource ID of the configuration to retrieve.')
...
class DeleteTaskPushNotificationConfigRequest(A2ABaseModel):
    ...
    tenant: str = Field(default='', description='Optional. Tenant ID, provided as a path parameter.')
    task_id: str = Field(..., description='The parent task resource ID.')
    id: str = Field(..., description='The resource ID of the configuration to delete.')
...
class SubscribeToTaskRequest(A2ABaseModel):
    ...
    tenant: str = Field(default='', description='Optional. Tenant ID, provided as a path parameter.')
    id: str = Field(..., description='The resource ID of the task to subscribe to.')
...
class ListTaskPushNotificationConfigsRequest(A2ABaseModel):
    ...
    tenant: str = Field(default='', description='Optional. Tenant ID, provided as a path parameter.')
    task_id: str = Field(..., description='The parent task resource ID.')
    page_size: int = Field(default=0, description='The maximum number of configurations to return.')
    page_token: str = Field(default='', description='A page token received from a previous `ListTaskPushNotificationConfigsRequest` call.')
...
class StreamResponse(A2ABaseModel):
    ...
    payload: Task | Message | TaskStatusUpdateEvent | TaskArtifactUpdateEvent | None = None
...
class ListTaskPushNotificationConfigsResponse(A2ABaseModel):
    ...
    configs: list[TaskPushNotificationConfig] | None = Field(default=None, description='The list of push notification configurations.')
    next_page_token: str = Field(default='', description='A token to retrieve the next page of results, or empty if there are no more results in the list.')
